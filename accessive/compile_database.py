...
def current_species_manifest(sqlite_file):
    conn = sqlite3.connect(sqlite_file)
    c = conn.cursor()
    c.execute('SELECT * FROM species_table')
    species = [x[1] for x in c.fetchall()]
    c.close()
    conn.close()
    return species 
...
def compile_full_database(sqlite_file, include_list=None, cache_dir=None):
    if cache_dir is None:
        cache_dir = tempfile.mkdtemp()

    assert(os.path.exists(cache_dir))
    already_loaded = current_species_manifest(sqlite_file)
